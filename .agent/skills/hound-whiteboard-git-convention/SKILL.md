...
9. **不用破折号**（——）：标题与正文都不用，需要补充说明时改用逗号或括号
10. **不用 `<file>: xxx` 前缀**：条目直接写变更内容，文件名放在句中自然出现或省略（如「更新操作文档的 WARNING」而非「operation-document.md：更新 WARNING」）
11. **标题带动词**：中文描述以动词开头或含明确动词（如「实现会话恢复」「修复幽灵选择」），不用裸名词短语
