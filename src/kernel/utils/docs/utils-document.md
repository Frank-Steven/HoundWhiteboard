...
- 数学与几何：`math.js`、`math-algorithm.js`
- 逻辑路径：`path.js`
...
- `math.js` 和 `math-algorithm.js` 负责几何表达与交互计算。
- `queue.js`、`directed-graph.js` 负责提供基础容器。
...
- 若处理区块缓冲区、BFS 或任务排队，优先阅读 [queue-document.md](./queue-document.md)。
