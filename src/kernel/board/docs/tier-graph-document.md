...
将被选择的对象所对应的点集记为 $\mathbb{A}$（活动点集）。选择过程分四步：

```mermaid
flowchart LR
    A["① 提取子图<br/>从 S 中提取以 A 为源点的<br/>最大可达子图 G"] --> B["② 计算层数<br/>取各路径上活动点<br/>计数的最大值"]
    B --> C["③ 构造新层<br/>活动点入活动对象集<br/>非活动点入非活动子图（仅留同层边）"]
    C --> D["④ 合并入动态图<br/>按层数升序并入 D<br/>（旧层之下 + 唯一性约束）"]
```
