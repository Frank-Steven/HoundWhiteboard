...
- 「原位置」节点：跨节点撤消改挂后留在原处的节点，与新位置的节点凭同一 share id 共享数据与逻辑身份（节点无副本级 id，share id 即落盘与跨端的逻辑身份），仅父子位置（所在分支）不同。

## 规则约定

- **操作为王**：操作是一等记录（id、author、时间、内容），可产生多个节点；树由操作的结构效果按 (时间, author) 序构建，是日志的追溯结构（文中 (时间, author) 为全序简写，完整决胜为 时间 → author → 操作序号，见[时间回溯树内核文档](undo-tree-kernel-document.md)「时间标记」）。
- **节点不持有数据**：数据放在数据池中凭 share id 获取；多个节点可凭同一 share id 共享数据。
- HEAD 指向当前状态对应的节点，可在链中间（其下游后代是已撤销的节点，可重做）；随操作应用自然移动，由日志派生、不参与仲裁。
- **撤销与重做（统一语义）**：撤销记录目标节点，应用时按目标在活动链上的位置呈现两种形态——目标为末端时退化为 HEAD 回退，目标在链中间时分叉改挂；重做凭 `targetUndoId` 引用一条已生效的撤销记录，把其目标节点按时间标记重新激活（插回活动链）。各情景表述中的「跨节点撤消」即撤销的一般形态（目标在链中间），「非跨节点撤消」即退化形态（目标为末端）。
- 操作的应用分两部分：结构变更（增删节点、改挂、撤销目标的重新激活）无条件应用；重做凭 `targetUndoId` 的登记册谓词判定生效（撤销已生效、未重做、未被同源新工作洗刷）；更改 HEAD 指针条件应用——仅当评估时 HEAD 恰为该操作的起点时才移动指针。
...
操作描述：O1 是单条分子操作记录（t=4, A，type undo，改挂类），目标是 A2（非 HEAD 所指）。应用一步完成分叉改挂与 HEAD 移动，不另产 move-head 日志条目：

- 复制挂接：在 A2 的父节点 B1 处分叉——B2 的节点（连同身份与子树）改挂到 B1 下；A2 原位置留下一个节点，与改挂后的节点凭同一 share id 共享数据。
- HEAD 移动：起点 B2（原位置），终点 B2（新位置）。HEAD 移动是撤销记录应用时算出的效果，条件应用，不产生节点。
...
3. **应用 HEAD 效果（条件）**：O1 的 HEAD 移动起点是 B2（原位置）；本地 HEAD 是 B3，不等于起点——指针不动。
...
| t=4  | A      | O1   | 撤销           | 撤销 A2                       | B1 处分叉，B2 改挂到 B1 下；原位置截断保留 B2 | t=6              |
| t=5  | B      | O2   | 增加对象       | 新增 B3（父 B2）              | 追加到 B2 后                                  | t=6              |
...
- 两个 HEAD 移动类操作并发的规则：已由情景三覆盖（按日志序回放评估）。
...
撤销记录目标节点，目标为活动链末端时退化为纯 HEAD 回退（本情景各撤销均呈退化形态，不改动树结构）；重做凭 `targetUndoId` 把撤销目标重新激活。两者都不产生节点。
...
两端各自撤销后 HEAD 都在 A2；收到对方的撤销操作时，B2 已不在活动链上——被吸收（不产生结构变化）。两端 HEAD = A2。同一撤销意图的重复操作被目标节点语义自然吸收。最终树（两端一致）：
...
- A 收 O2：B2 已不在活动链上，被吸收；收 O3：目标 A2 在活动链上（A2 为末端），应用，HEAD → B1。
- B 收 O1：B2 已不在活动链上（本地 O2 已将其撤下），被吸收；日志序重估后 O1 生效、O2 被吸收（见情形 3 的重放规则）。

两端 HEAD = B1。撤销效果取并集（最深者）——目标节点语义让连续的撤销级联生效。最终树（两端一致）：
...
| ----- | ------ | ---- | ---- | -------------------------- | ---------------------------------------------- | ---------------- |
| t=4   | A      | O1   | 撤销 | 撤销 B2                    | 退化为 HEAD 回退（B2 → A2）                    | t=6              |
| t=5   | B      | O2   | 撤销 | 撤销 B2                    | 日志序上被吸收（B2 已被 O1 撤下），不登记      | t=6              |
| t=5.5 | A      | O3   | 重做 | 重做 O1（targetUndoId=O1） | B2 重新激活（追加回链末端，HEAD A2 → B2）      | t=6              |
| t=6   | —      | —    | —    | 互相同步                   | —                                              | —                |

日志序 O1(t4) → O2(t5) → O3(t5.5)：O1 生效并登记（B2 为末端，退化为 HEAD 回退）→ O2 被吸收（B2 已不在活动链上；被吸收的撤销不产生重做目标，B 端 redo() 无可重做目标、不发记录）→ O3 凭 targetUndoId 查登记册：O1 已生效、未重做、未被 A 的新工作洗刷——生效，B2 重新激活回链末端。两端 HEAD = B2：

- A 端顺序：O1（本地生效）→ O2（收到时 B2 已撤，吸收）→ O3（本地生效）→ HEAD = B2。
- B 端顺序：O2（本地先生效：B2 撤下）→ O1（树级操作乱序到达，插入点之后有已应用的 O2——后缀重放，按日志序重估：O1 生效、O2 被吸收）→ O3（B2 重新激活）→ HEAD = B2。

同一组操作按不同顺序到达，两端各自增量求值、以日志序重放兜底，收敛到同一棵树、同一 HEAD。最终树（两端一致）：
...
2. **HEAD 移动类按日志序回放评估**：更改 HEAD 指针的条件应用与重做的登记册谓词都引用日志前缀的派生态，评估顺序必须是全局 (时间, author) 序，不能是到达顺序，否则情形 3 发散。HEAD 与树同构——树 = 日志的确定性函数，HEAD 与登记册同为日志前缀的确定性函数。
3. **目标节点语义的吸收性**：重复与级联的撤销在日志序回放中自然得到正确结果（情形 1、2）。
4. **统一语义下的情形 3**：撤销统一为目标节点语义后，A、B 的撤销都记为「撤销(B2)」——日志序回放中 A 的撤销先生效（B2 为末端，退化为 HEAD 回退），B 的撤销被吸收（不产生重做目标）；A 重做自己的撤销（targetUndoId = O1），B2 重新激活，两端 HEAD = B2。
...
| t=5  | A      | O1   | 撤销           | 撤销 A2              | B1 处分叉，B2 → X 改挂到 B1 下；原位置截断保留 B2、X | t=6              |
| t=6  | —      | —    | —              | O1 到 B              | —                                                    | —                |
| t=7  | —      | —    | —              | X 到 A，触发后缀重放 | —                                                    | —                |
...
| t=4  | A      | O1   | 撤销           | 撤销 B1            | A1 处分叉，C1 → A2 改挂到 A1 下；原位置截断保留 C1、A2 | t=6              |
...
| t=4  | A      | O2   | 重做     | 重做 O1（targetUndoId=O1）            | C1 重新激活（追加回链末端，HEAD B1 → C1）  | t=7              |
...
A 端（t=4 后）：O1 撤 C1——C1 为末端，退化为 HEAD 回退（HEAD = B1）；O2 重做——O1 已登记、未被 A 的新工作洗刷，C1 重新激活（其时间晚于链末端 B1，追加并推进 HEAD），HEAD = C1：
...
1. **回放路径（树 = f(日志)，HEAD = g(日志)）**：O1(t=3) 撤 C1（退化，HEAD = B1，登记）→ O2(t=4) 重做 O1（C1 重新激活，HEAD = C1）→ O3(t=5) 撤 C1（退化，HEAD = B1——B 的撤销是最后生效的意图）→ O4(t=6)：C2 的父 C1 不在活动链上（已撤销），按 (时间, author) 插入活动链末端，HEAD = C2。

2. **A 端增量视角**（本地先有 O1、O2）：收 O3(t=5)——日志序最新，纯应用：C1 为活动链末端，退化为 HEAD 回退，HEAD = B1；收 O4(t=6)——C2 按时间插入活动链末端（B1 后），HEAD = C2。全程纯应用。

3. **B 端增量视角**（本地先有 O3）：收 O1(t=3)——插入点之后有已应用的 O3（撤销，上下文敏感），重放受影响后缀：O1（退化，HEAD = B1，登记）→ O2（重做：C1 重新激活，HEAD = C1）→ O3（退化，HEAD = B1），与回放路径一致；收 O4(t=6)——时间插入，HEAD = C2。

4. **C 端增量视角**（本地先有 O4）：收 O1(t=3)——插入点之后只有 O4（增加节点类），纯应用：C1 在链中间（C2 在其上）呈一般形态，B1 处分叉、C2 改挂到 B1 下，截断判定 C2(t=6) > O1(t=3) 不留原位置，HEAD = C2；收 O2(t=4)——插入点之后有已应用的 O4（增加节点类）：重做的生效判定引用日志前缀派生的登记册，按「插入与重放的边界」重放受影响后缀——O2 按日志序重估生效（C1 重新激活回 B1 之后），O4 在其日志位置重新追加，HEAD = C2；收 O3(t=5)——插入点之后只有 O4（增加节点类），纯应用：C1 为活动链末端，退化为 HEAD 回退，HEAD = B1；O4 的 HEAD 推进在其日志位置生效，最终 HEAD = C2，与回放路径汇合：
...
3. **重做与更改 HEAD 的求值由后缀重放保证**：重做的生效判定引用日志前缀派生的生效撤销登记册——其插入点之后有任何已应用操作（含增加节点类）时增量求值可能失真，一律后缀重放（见「插入与重放的边界」）；本情景 C 端收 O2 即为例证。
...
| t=5  | A      | O3   | 重做     | 重做 O1（targetUndoId=O1）            | A2 重新激活，按时间标记插回 B1 与 B2 之间    | t=6              |
| t=6  | —      | —    | —        | 互相同步                          | —                                      | —                |

### 同步前的各自视角

A 端（t=5 后）：O1 撤 A2——A2 为末端，退化为 HEAD 回退（HEAD = B1）；O3 重做——A 端尚不知 B2，A2 重新激活追加回链末端，HEAD = A2：
...
1. **回放路径（树 = f(日志)，HEAD = g(日志)）**：O1(t=3) 撤 A2（退化，HEAD = B1，登记）→ O2(t=4)：B2 的父 A2 不在活动链上（已撤销），按时间插入活动链末端（B1 后），HEAD = B2 → O3(t=5)：O1 已生效登记、未重做、未被**同源**新工作洗刷（O2 是 B 的工作，不洗刷 A 的重做资格）——生效：A2 重新激活，按时间标记（t=2）插回 B1 与 B2 之间，HEAD = B2 不动。

2. **A 端增量视角**（本地先有 O1、O3，HEAD = A2）：收 O2(t=4)——插入点之后有已应用的 O3（重做，上下文敏感），重放受影响后缀：O2 按时间插入活动链（A1 → B1 → B2，HEAD = B2），O3 重估仍生效——A2 重新激活插回 B1 与 B2 之间。HEAD = B2——远端并发新增不再冲掉本端重做，重做的操作记录留在日志中、各端一致生效。

3. **B 端增量视角**（本地先有 O2，HEAD = B2）：收 O1(t=3)——插入点之后只有 O2（增加对象，上下文不敏感），纯应用：A2 在链中间（B2 在其上）呈一般形态，B1 处分叉、B2 改挂到 B1 下，截断判定 B2(t=4) > O1(t=3) 不留原位置；收 O3(t=5)——插入点之后无已应用操作，纯应用：O1 已登记未洗刷，A2 重新激活插回 B1 与 B2（改挂副本）之间。结果与回放路径相同。

两端路径不同（A 端重放、B 端纯应用），汇合到同一棵树：

```mermaid
flowchart LR
    R((A&B)) --> A1["A1(t0, A)"] --> B1["B1(t1, B)"] --> A2["A2(t2, A)"] --> B2(["B2(t4, B)"])
```

### 结果

- 两端收敛到同一棵树、同一 HEAD（B2），状态 = A1、B1、A2、B2。

### 设计要点确认

1. **远端新工作不洗刷重做**：重做凭 `targetUndoId` 引用生效撤销，只有**同源**新工作洗刷本端的重做资格（经典编辑器的重做语义）；远端并发工作留在链上不动，被撤节点按时间标记插回其位置——各端按同一时间序评估，结论一致，收敛不依赖投递交错。
2. **发射即生效**：本端 redo() 先查资格再发射（无资格不产生记录）；一旦记录，各端按日志必然得出相同的生效结论——不会出现「本端不发效果、对端生效」的永久分歧。
...
- **局部求值**：增加节点类（效果快照自包含，位置由 (时间, author) 决定）、撤销（形态与改挂范围由目标的 ≤U / >U 后继决定，截断保证殊途同归）、close-supra（折叠范围由评估时活动链上同 supraId 的连续段决定）——求值只读当前树与时间标记，不引用日志前缀的折叠值。
- **引用 g(前缀)**：重做（生效判定引用日志前缀派生的生效撤销登记册）、更改 HEAD 指针（条件应用求值「评估时的 HEAD」，HEAD 是日志前缀的折叠值）——任何前缀内的操作都会改变求值环境。
...
| 增加节点类      | 有撤销 / 重做 / 更改 HEAD / close-supra | 后缀重放：新节点改变已应用操作的求值上下文（情景四、五）   |
| 撤销            | 无，或只有增加节点类                | 纯应用：当前树结构效果 + 截断（情景一、七、九）                |
| 撤销            | 有撤销 / 重做 / 更改 HEAD / close-supra | 后缀重放                                                   |
| close-supra     | 无，或只有增加节点类                | 纯应用：活动链上同 supraId 的连续段折叠为聚合节点              |
| close-supra     | 有撤销 / 重做 / 更改 HEAD / close-supra | 后缀重放                                                   |
| 重做、更改 HEAD | 无                                  | 纯应用：登记册 / 条件求值                                             |
| 重做、更改 HEAD | 有任何已应用操作                    | 后缀重放：求值引用日志前缀派生态（情景八）                       |

重放范围 = 插入点到日志末尾的受影响后缀，按日志序重新应用：撤销的形态重估、close-supra 的折叠重估、HEAD 移动的条件重估、增加节点类的结构位置随后缀一并整理。
...
| 九（A 端收 O2，新增）        | 插入点之后有已应用的 O3（重做）       | 后缀重放，O3 重估仍生效 ✓                        |
