...
>
> 注记时曾存在的「实况提交与重放派生的 z-order 语义差」已随层位边效果记录消除：对象操作记录携带提交/提取时刻的层位边（below/above），回放与远端直接应用记录边而不做几何重算，记录之外的对象按后到者居上缝合。同夹具复测（2026-08-15）：相交对层位反转为 0（两路视觉等价），非相交对的绘制序换位属插入序副产物、视觉无关。旧日志形态（记录未携带层位边）回退几何居上派生。
