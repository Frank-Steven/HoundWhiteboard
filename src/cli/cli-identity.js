...
async function resolveCliIdentity() {
  const file = cliIdentityFile();
  try {
    const desc = JSON.parse(await fs.readFile(file, "utf-8"));
    if (typeof desc?.source === "string" && desc.source.startsWith("cli-")) {
      return desc.source;
    }
  } catch {
    /* 缺失或损坏时按首启处理 */
  ...
  const source = `cli-${random.toString(36).padStart(4, "0")}`;
  await fs.mkdir(path.dirname(file), { recursive: true });
  const tmp = `${file}.tmp-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  await fs.writeFile(tmp, JSON.stringify({ source }), "utf-8");
  await fs.rename(tmp, file);
  return source;
}

export { resolveCliIdentity, cliIdentityFile };
