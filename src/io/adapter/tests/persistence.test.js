...
describe("createPersistenceAdapter", () => {
  ...
  test("save/load 对象批量往返", async () => {
    const { adapter } = setup();
    const objects = [
      { id: 1, type: "StrokeObject", points: [1, 2, 3] },
      { id: 2, type: "Container", children: [] },
    ];

    expect(await adapter.saveObjects(objects)).toBe(true);

    const loaded = await adapter.loadObjects([1, 2]);
    expect(loaded).toHaveLength(2);
    expect(loaded.find((o) => o.id === 1).points).toEqual([1, 2, 3]);
  });

  test("loadObjects 跳过缺失对象", async () => {
    const { adapter } = setup();
    await adapter.saveObjects([{ id: 1, type: "X" }]);
    const loaded = await adapter.loadObjects([1, 2, 3]);
    expect(loaded.map((o) => o.id)).toEqual([1]);
  });

  test("存储布局与旧桥兼容（chunks/{id}.json、objects/{id}.json）", async () => {
    const { driver, adapter } = setup();
    await adapter.saveChunkMetadata(7, { tierGraph: ["t"], objectCoverIndex: [] });
    await adapter.saveObjects([{ id: 42, type: "Y" }]);

    const chunkRaw = await driver.read(ROOT_ID, "chunks/7.json");
    expect(JSON.parse(chunkRaw).tierGraph).toEqual(["t"]);

    const objectRaw = await driver.read(ROOT_ID, "objects/42.json");
    expect(JSON.parse(objectRaw).type).toBe("Y");
  });

  test("deleteObject 删除对象文件", async () => {
    const { adapter } = setup();
    await adapter.saveObjects([{ id: 5, type: "Z" }]);
    expect(await adapter.deleteObject(5)).toBe(true);
    expect(await adapter.loadObjects([5])).toEqual([]);
  });

  test("损坏的对象 JSON 被跳过", async () => {
    const { driver, adapter } = setup();
    await driver.write(ROOT_ID, "objects/9.json", "{oops");
    expect(await adapter.loadObjects([9])).toEqual([]);
  });
